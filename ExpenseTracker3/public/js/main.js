...
let request; // declaring a variable to store requests which will be sent to the server.
  ...
  getList(); // sending a request to the server to return the expense list, and then update the variables localList and listSize accordingly as well as the expense list on the server.
...
function updateTable() {
  ...
  for (let x = 1; x <= listSize; x++) { // looping for as many times as there are expenses/entries, in order to display each one as a row in the table.
    ...
    */
    cell2.innerHTML = localList[x-1].item_name;

    /* updating cell to display the total cost of all expenses , using the same method in the previous line
    */
    cell3.innerHTML = "€" + parseFloat(localList[x-1].item_cost).toFixed(2);
    ...
    // The 'Delete' button is set to run the removeExpense(ID) function
     ...
     total = total + parseFloat(localList[x-1].item_cost); // adding the cost of all entries in the total variable
  }
...
function getRemoveButtonClicked(ID){
  ...
  window.location.reload(); // reloading the main page
